  ...
  <view>
    ...
    <view class="video-list song-list">
      ...
      <view class="video-item" v-for="(item, index) in relatedVideo" :key="index">
        <image :src="item.coverUrl" mode="aspectFit" class="img"></image>
        <view class="desc ellipsis">
          {{item.title}}
        </view>
      </view>
...
<script setup>
  ...
  const getBanner = () => {
    apisIndex.apiGetBanner().then(res => {
  ...
  // 新碟新歌 将前3首歌--新碟，后3首--新歌
  let newType = ref(1) // 新碟新歌类别 1--第一个 2--第二个
  let state = reactive({
    latestAlbum: [],  // 存储新碟新歌
  ...
  const getLatestAlbum = () => {
    apisIndex.apiGetTopAlbum().then(res => {
      // 将所有的数据暂存在临时变量中
